...
const Navbar = () => {
    ...
    const { cart } = UseCart()
    const [theme, setTheme] = useState(localStorage.getItem('couture-theme') ? localStorage.getItem('couture-theme') : 'night')
        ...
        <div className={!isNight ? "bg-[#ffffff5e] fixed w-full backdrop-blur-sm z-10 shadow-md" : "bg-[#00000056] fixed w-full backdrop-blur-sm z-10 shadow-md"}>
            <div className=" my-container lg:flex justify-between hidden py-5">
                ...
                <ul className="flex gap-5 items-center text-lg">
                    ...
                    <NavLink className={({ isActive }) => isActive ? 'border-b-4 border-[#03e9a4] px-1' : 'px-1 border-b-4 border-transparent'} to='/dashboard/profile'>Dashboard</NavLink>
                    {
                        role === 'student' &&
